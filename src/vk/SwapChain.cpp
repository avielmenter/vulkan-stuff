...
void SwapChain::createSwapChain(const VkSurfaceFormatKHR &format, const VkPresentModeKHR &presentMode, const VkExtent2D &extent, uint32_t numImages)
{
	...
	this->formatInUse = format;	

	vkGetSwapchainImagesKHR(this->device.getVkDevice(), this->swapChain, &numImages, nullptr);
	this->swapChainImages.resize(numImages);
	vkGetSwapchainImagesKHR(this->device.getVkDevice(), this->swapChain, &numImages, this->swapChainImages.data());
}

void SwapChain::createImageViews()
{	
	this->swapChainImageViews.resize(this->swapChainImages.size(), VkDeleter<VkImageView> { this->device.device, vkDestroyImageView });
	for (size_t i = 0; i < this->swapChainImages.size(); i++)
	{
		...
		createInfo.format = this->formatInUse.format;
		...
		if (vkCreateImageView(this->device.getVkDevice(), &createInfo, nullptr, this->swapChainImageViews[i].replace()) != VK_SUCCESS)
